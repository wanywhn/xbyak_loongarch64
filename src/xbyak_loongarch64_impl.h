...
void CodeGenerator::LoongarchOnlyOpcode(uint32_t opCode) { dd(opCode); }
...
template <typename T> void CodeGenerator::LasxFormat4R(uint32_t op, uint32_t df, const T &ra, const T &rk, const T &rj, const T &rd) {
  uint32_t code = concat({F(op, 22), F(df, 20), F(ra.getIdx(), 15), F(rk.getIdx(), 10), F(rj.getIdx(), 5), F(rd.getIdx(), 0)});
  dd(code);
}

template <typename T> void CodeGenerator::LasxFormatCond(uint32_t op, uint32_t df, uint32_t cond, const T &rk, const T &rj, const T &rd) {
  uint32_t code = concat({F(op, 22), F(df, 20), F(cond, 15), F(rk.getIdx(), 10), F(rj.getIdx(), 5), F(rd.getIdx(), 0)});
  dd(code);
}

template <typename T, typename R> void CodeGenerator::LasxFormatI12(uint32_t op, int32_t imm, const R &rj, const T &rd) {
...
template <typename T, typename R> void CodeGenerator::LasxFormatAddr(uint32_t op, uint32_t df, int32_t off, const R &rj, const T &rd) {
  uint32_t df_off = 0;
...
template <typename T, typename R> void CodeGenerator::LasxFormatI8ELM(uint32_t op, uint32_t df, uint32_t idx, int32_t imm8, const R &rj, const T &rd) {
  uint32_t df_n = 0;
...
template <typename T, typename R> void CodeGenerator::LasxFormat3RX(uint32_t op, const R &rk, const R &rj, const T &rd) {
  uint32_t subOp = 0b000;
  uint32_t code = concat({F(op, 18), F(subOp, 15), F(rk.getIdx(), 10), F(rj.getIdx(), 5), F(rd.getIdx(), 0)});
  dd(code);
}

template <typename T, typename R> void CodeGenerator::LasxFormat3R(uint32_t op, uint32_t df, const R &rk, const T &rj, const T &rd) {
  uint32_t code = concat({F(op, 17), F(df, 15), F(rk.getIdx(), 10), F(rj.getIdx(), 5), F(rd.getIdx(), 0)});
  dd(code);
}

template <typename T> void CodeGenerator::LasxFormat3RVec(uint32_t op, const T &rk, const T &rj, const T &rd) {
  uint32_t code = concat({F(op, 15), F(rk.getIdx(), 10), F(rj.getIdx(), 5), F(rd.getIdx(), 0)});
  dd(code);
}

template <typename T, typename S, typename std::enable_if<std::is_signed<S>::value, std::nullptr_t>::type> void CodeGenerator::LasxFormatI5(uint32_t op, uint32_t df, S imm5, const T &rj, const T &rd) {
...
template <typename T, typename S, typename std::enable_if<std::is_unsigned<S>::value, std::nullptr_t>::type> void CodeGenerator::LasxFormatI5(uint32_t op, uint32_t df, S imm5, const T &rj, const T &rd) {
...
template <typename T> void CodeGenerator::LasxFormatI5Mode(uint32_t op, uint32_t imm5, uint32_t mode, const T &rd) {
  verifyIncRange(imm5, 0, ones(5), ERR_ILLEGAL_IMM_RANGE, false);
...
template <typename T, typename R> void CodeGenerator::LasxFormat2R(uint32_t op, uint32_t df, const R &rj, const T &rd) {
  uint32_t code = concat({F(op, 12), F(df, 10), F(rj.getIdx(), 5), F(rd.getIdx(), 0)});
  dd(code);
}

template <typename T> void CodeGenerator::LasxFormatCFR(uint32_t op, uint32_t df, const T &rj, uint32_t cd) {
...
template <typename T> void CodeGenerator::LasxFormatBIT(uint32_t op, uint32_t df, uint32_t m, const T &rj, const T &rd) {
  uint32_t df_m = 0;
...
template <typename T, typename R> void CodeGenerator::LasxFormatELM(uint32_t op, uint32_t df, uint32_t idx, const T &rj, const R &rd) {
  uint32_t df_n = 0;
...
template <typename T, typename R> void CodeGenerator::LasxFormatELM_1(uint32_t op, uint32_t df, uint32_t idx, const T &rj, const R &rd) {
  uint32_t df_n = 0;
...
template <typename T> void CodeGenerator::LasxFormat2RSubOp(uint32_t op, uint32_t subOp, const T &rj, const T &rd) {
  uint32_t code = concat({F(op, 18), F(subOp, 10), F(rj.getIdx(), 5), F(rd.getIdx(), 0)});
  dd(code);
}

template <typename T> void CodeGenerator::LasxFormatI8(uint32_t op, uint32_t imm8, const T &rj, const T &rd) {
...
template <typename T> void CodeGenerator::LasxFormatI13(uint32_t op, int32_t imm13, const T &rd) {
