...
enum simdLen_t { SIMD_NONE = 0, LSX_MASK = 1 << 6, LASX_MASK = 1 << 7 };
...
public:
  ...
  int cpucfg_test(void) {
    ...
    if (pid == 0) { /* Subprocess */
      ...
      __asm__ volatile("cpucfg $t0, $zero    \n\t" :::);
      g_support_cpucfg = 1;
      ...
      close(g__fd[1]);
      if ((waitpid(pid, &status, 0) <= 0) || (read(g__fd[0], &g_support_cpucfg, sizeof(g_support_cpucfg)) <= 0))
        g_support_cpucfg = 0;
      close(g__fd[0]);
    } else {
      g_support_cpucfg = 0;
  ...
  uint64_t get_cpu_flags_from_cpucfg() {
    uint64_t flags = 0;
    int flag = 0;
    __asm__ volatile("addi.d $a6, $zero, 0x02        \n\t"
                     "cpucfg $a7, $a6        \n\t"
                     "st.w   $a7, %0, 0x00   \n\t"
                     :
                     : "r"(&flag)
