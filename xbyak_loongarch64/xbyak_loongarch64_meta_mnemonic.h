...
template <typename T, typename std::enable_if<std::is_unsigned<T>::value, std::nullptr_t>::type = nullptr> void add_imm(const XReg &dst, const XReg &src, T imm, const XReg &tmp) {
  ...
  // si16<<16
  else if ((bit_ptn & 0xffff) == 0 && (bit_ptn & 0x80000000) == 0 && (bit_ptn >> 32) == 0) {
...
template <typename T, typename std::enable_if<std::is_signed<T>::value, std::nullptr_t>::type = nullptr> void add_imm(const XReg &dst, const XReg &src, T imm, const XReg &tmp) {
  ...
  if ((bit_ptn & 0xffff) == 0 && ((signBit == 0 && upper32 == 0) || (signBit != 0 && upper32 == 0xffffffff))) {
    addu16i_d(dst, src, (int32_t)(bit_ptn >> 16));
    return;
  }
...
template <typename T, typename std::enable_if<std::is_unsigned<T>::value, std::nullptr_t>::type = nullptr> void mov_imm(const XReg &dst, T imm) {
  uint64_t bit_ptn = static_cast<uint64_t>(imm);
...
template <typename T, typename std::enable_if<std::is_signed<T>::value, std::nullptr_t>::type = nullptr> void mov_imm(const XReg &dst, T imm) {
  int64_t bit_ptn = static_cast<int64_t>(imm);
